...
impl SQLdb {
    ...
    pub fn set_ongoing_session(&mut self, s_id: &SessionId) {
        println!("Set ongoing session: {}", s_id);
    ...
    pub fn next_session(&mut self) -> bool {
        ...
        if self.co.len() > 0 {
            let first = self.co.remove(0);
            println!{"REPLAY Next: {:?}", first};
            self.set_ongoing_session(&first.0);
            return true;
        }
    ...
    // ISOPredict
    pub fn load_traces(&mut self) {
        for (s_id, trace) in &self.traces {
            self.dis_kv.traces.insert(*s_id, trace.to_vec());
        }

        for (s_id, reads) in &self.tx_reads {
            println!("REPLAY Session[{}] # of READS: {:?}", s_id, reads);
            self.dis_kv.tx_reads.insert(*s_id, reads.to_vec());
        }
    }
    ...
    pub fn execute<W: std::io::Write>(
            ...
            | sql.starts_with("SHOW WARNINGS")
        {
            ...
            match parsed_sql {
                ...
                Err(error) => {
                    // ISOPredict
                    if sql.contains("ISOLATION") {
                        self.terminals += 1;
                        if self.terminals == self.nodes && self.co.len() > 0 {
                            println!("Starting REPLAY");
                            self.replay = true;
                        } else {
                            println!("Num of Terminals = {}", self.terminals);
                        }
                    }
                    println!("{:?}", error)
                },
        ...
        match parsed_sql {
            Ok(v) => match &v[0] {
                ...
                Commit { .. } => {
                    let non_empty_tx = self.commit(session_id);
                    if !self.replay || non_empty_tx {
    ...
    pub fn select<W: std::io::Write>(
        ...
        match query.body {
            ast::SetExpr::Select(ref s) => {
                ...
                'outer: for e_key in prim_key_sorted.drain(..) {
                    ...
                    if let Some(ref selection) = s.selection {
                        ...
                        // ISOPredict
                        let mut select_vars_sorted: Vec<_> = select_vars.into_iter().collect();
                        select_vars_sorted.sort_unstable();

                        for proj in &select_vars_sorted {
    ...
    pub fn update<W: std::io::Write>(
        ...
        for e_key in prim_key_sorted.iter() {
            let mut selected = true;
    ...
    pub fn delete<W: std::io::Write>(
        ...
        for prim_key in prim_key_sorted.iter() {
