...
pub struct KVStore {
    ...
    // ISOPredict
    pub map: HashMap<VarId, TxnId>,
    pub setmap: HashMap<SetId, HashMap<VarId, (u64, u64)>>,
    pub traces: HashMap<u64, Vec<(u64, u64, u64)>>,
    pub tx_reads: HashMap<u64, Vec<usize>>,
    pub n_read: usize,
...
impl KVStoreT for KVStore {
    fn commit(&mut self, s_id: &SessionId) -> bool {
        ...
        if let Some(txn) = entry.last_mut() {
            txn.commit();
            // ISOPredict
            // print all the operations within a transaction
            for op in &txn.op {
                cnt += 1;
                match op {
                    Op::Write(key, _value, (tx, _seq)) => {
                        println!("WRITE KEY[{:?}] Txn{:?}", key, tx);
                        self.map.insert(*key, tx.clone());
                    },
                    Op::Read(key, (from, _from_seq), (tx, _seq)) =>  {
                        println!("READ KEY[{:?}] Txn{:?} From{:?}", key, tx, from);
                    },
                    Op::Insert(set_id, key, (tx, _seq)) => {
                        println!("INSERT[{:?}] to Set[{:?}] Txn{:?}", key, set_id, tx);
                        let ent = self.setmap.entry(*set_id).or_default();
                        ent.insert(*key, tx.clone());
                    },
                    Op::Delete(set_id, key, (tx, _seq)) => {
                        println!("DELETE[{:?}] from Set[{:?}] Txn{:?}", key, set_id, tx);
                        let ent = self.setmap.entry(*set_id).or_default();
                        ent.insert(*key, tx.clone());
                    },
                    Op::Contains(set_id, key, (from, _from_seq), (tx, _seq)) => {
                        println!("CONTAINS[{:?}] in Set[{:?}] From{:?} Txn{:?}", key, set_id, from, tx);
                    },
                }
            }

            let mut curr_read: usize = 0;
            let tx_reads = self.tx_reads.entry(*s_id).or_default();
            if tx_reads.len() > 0 {
                curr_read = tx_reads.remove(0);
            }

            self.total_replayed += self.n_replayed;
            self.total_read += curr_read;

            // remove entries in the trace that are read by current transaction
            let trace = self.traces.entry(*s_id).or_default();
            for _i in 0..curr_read {
                if trace.len() > 0 {
                    trace.remove(0);
                }
            }
            ...
            if self.total_read > 0 {
                println!("REPLAY matched {}/{} at Session {}. Total matches: {}/{}", self.n_replayed, curr_read, *s_id, self.total_replayed, self.total_read);
            }
    ...
    fn contains(&mut self, s: &SetId, x: &VarId, s_id: &SessionId) -> bool {
        let r_t_id = if let Some((_, op_id)) = self.get_last_transaction_mut(s_id).contains(s, x) {
            ...
            let trace = self.traces.entry(*s_id).or_default();
            let tx_reads = self.tx_reads.entry(*s_id).or_default();
            let replay_reads = if tx_reads.len() > 0 {
                tx_reads[0]
            } else {
                0
            };
            ...
            // ISOPredict
            if trace.len() > self.n_read && replay_reads > self.n_read{
                let replay = trace.get(self.n_read).unwrap();
                let rc = (replay.0, replay.1);
                // println!("REPLAY SESSION[{}] KEY[{}] {:?} at {}", s_id, x, replay, self.n_read);
                ...
                if *x == replay.2 {
                    ...
                    // println!("CONTAINS Set:{} Key:{} Session:{} Replay:{:?} was not part of the choices", s, x, s_id, replay);

                    // return an Serial choice
                    let serial = if let Some(v) = self.setmap.get(s) {
                        if let Some(txn) = v.get(x) {
                            *txn
                        } else {
                            (0, 0)
                        }
                    ...
                    if choices.contains(&serial) {
                        serial
                    } else {
                        println!("REPLAY KEY[Set({}:{})] fallback to MonkeyDB's choice at Session [{}], position [{}]", s, x, s_id, self.n_read);
                        if choices.len() > 1 {
                            self.choose_one_op(&choices)
                        } else if choices.len() == 1 {
                            choices[0]
                        } else {
                            println!("Empty choices vector");
                            serial
                        }
                    }
                }
        ...
        let r_t_id = if let Some((_, op_id)) = self.get_last_transaction_mut(s_id).read(x) {
            ...
            if trace.len() > self.n_read && replay_reads > self.n_read {
                ...
                if *x == replay.2 {
                    ...
                    // println!("READ Key:{} Session:{} Replay:{:?} was not part of the choices", x, s_id, replay);

                    // return an Serial choice
                    let serial = if let Some(txn) = self.map.get(x) {
                        *txn
                    } else {
                        (0, 0)
                    };
                    ...
                    if choices.contains(&serial) {
                        ...
                        println!("REPLAY KEY[{}] fallback to MonkeyDB's choice at Session [{}], position [{}]", x, s_id, self.n_read);
                        if choices.len() > 1 {
                            self.choose_one_op(&choices)
                        } else if choices.len() == 1 {
                            choices[0]
...
impl KVStore {
    ...
    pub fn inserted_once(&self, s1: &SetId) -> Vec<VarId> {
        ...
        // ISOPredict
        let mut all_vars_sorted: Vec<_> = all_vars.drain().collect();
        all_vars_sorted.sort_unstable();
        all_vars_sorted
    }
