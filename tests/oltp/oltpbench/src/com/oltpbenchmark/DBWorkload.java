...
public class DBWorkload {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // ISOPredict
        options.addOption("rng", "rngseed", true, "Seed of Random Number Generators");
        ...
        // ISOPredict
        int seedBase = 0;
        if (argsLine.hasOption("rng")) {
            seedBase = Integer.parseInt(argsLine.getOptionValue("rng"));
        }
        ...
        if (isBooleanOptionSet(argsLine, "execute")) {
            ...
            Results r = null;
            try {
                r = runWorkload(benchList, verbose, intervalMonitor, totalTx);
            } catch (Throwable ex) {
    ...
    private static Results runWorkload(List<BenchmarkModule> benchList, boolean verbose, int intervalMonitor, int totalTx) throws QueueLimitException, IOException {
        ...
        for (BenchmarkModule bench : benchList) {
            ...
            workers.addAll(bench.makeWorkers(verbose, totalTx));
            // LOG.info("done.");
        ...
        // ISOPredict
        Results r = ThreadBench.runDeterministicBenchmark(workers, workConfs);
