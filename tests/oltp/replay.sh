...
function oltp_run() {
    ...
    ${MONKEYDB_DIR}/target/release/monkeydb -a 0 -t ${TRACE} -n ${NTERM} >> "$MONKEYDB_LOG" 2>&1 &
    MONKEYDB_PID=$!
...
function setup_oltp_run {
    ...
    CURR_RUN_DIR=`mktemp -d $RUN_DIR/REPLAY_$(now)_XXX_run_${BENCHNAME}_${NTERM}_${DUR}_${CONSISTENCY}_${EXEC_ST}_${ASSERT_ST}_${SEED}`
    CURR_RUN=$(basename $TRACE)
    CURR_TRACE="REPLAY_${CURR_RUN%.*}"
    CURR_DIV="DIV_${CURR_RUN%.*}"
    CURR_READS="READS_${CURR_RUN%.*}"

    CURR_TRACE_DIR=${TRACE_DIR}/$BENCHNAME
    mkdir -p ${CURR_TRACE_DIR}

    CURR_DIVERGE_DIR=${DIVERGE_DIR}/$BENCHNAME
    mkdir -p ${CURR_DIVERGE_DIR}
    ...
    EXEC_TRACE=${CURR_TRACE_DIR}/${CURR_TRACE}.txt
    DIV_TRACE=${CURR_DIVERGE_DIR}/${CURR_DIV}.txt
    READS_TRACE=${CURR_DIVERGE_DIR}/${CURR_READS}.txt 
    OLTP_OUTD=oltp_out
    oltp_run

    grep -iE '(key|insert|delete|contains)\[.*\]' ${MONKEYDB_LOG} > ${EXEC_TRACE}
    grep 'matched' ${MONKEYDB_LOG} > ${DIV_TRACE}
    grep 'READS' ${MONKEYDB_LOG} > ${READS_TRACE}
}
...
function run_bench() {
    ...
    for i in `seq 1 ${total_run}`; do
        ...
        start=`date +%s`
        setup_oltp_run ${bench} ${nodes} "${timelimit}" "${consistency}" "${trace}" "${seed}" >> "${curr_violated_log_dir}/${bench}.out" 2>&1
        end=`date +%s`
        dur=$(($dur + $end - $start))
    done
    printf "\e[K" >&2
...
TRACE_DIR="${LOG_DIR}/validation_traces"
mkdir -p ${TRACE_DIR}

DIVERGE_DIR="${LOG_DIR}/divergence"
mkdir -p ${DIVERGE_DIR}
...
for node in ${nodes//,/ }; do
    for consistency in ${consistencies//,/ }; do
        for bench in ${benches//,/ }; do
            run_bench "$node" "$consistency" "$bench" "$total_run" "$seed" "$trace"
        done
    done
done
